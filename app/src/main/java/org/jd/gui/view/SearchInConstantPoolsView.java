...
public class SearchInConstantPoolsView<T extends DefaultMutableTreeNode & ContainerEntryGettable & UriGettable> {
    ...
    public SearchInConstantPoolsView(
        ...
        SwingUtil.invokeLater(() -> {
            searchInConstantPoolsDialog = new JDialog(mainFrame, "查找", false);//Search
            ...
            hbox.add(new JLabel("查找字符串(* = 任意字符串, ? = 任意字符):"));//Search string (* = any string, ? = any character)
            ...
            subpanel.setBorder(BorderFactory.createTitledBorder("查找"));//Search For
            ...
            subsubpanel.add(searchInConstantPoolsCheckBoxType = new JCheckBox("类型", true));//Type
            searchInConstantPoolsCheckBoxType.addItemListener(checkBoxListener);
            subsubpanel.add(searchInConstantPoolsCheckBoxField = new JCheckBox("字段"));//Field
            searchInConstantPoolsCheckBoxField.addItemListener(checkBoxListener);
            subhbox.add(subsubpanel);

            subsubpanel = new JPanel();
            subsubpanel.setLayout(new GridLayout(2, 1));
            subsubpanel.add(searchInConstantPoolsCheckBoxConstructor = new JCheckBox("构造方法"));//Constructor
            searchInConstantPoolsCheckBoxConstructor.addItemListener(checkBoxListener);
            subsubpanel.add(searchInConstantPoolsCheckBoxMethod = new JCheckBox("方法"));//Method
            searchInConstantPoolsCheckBoxMethod.addItemListener(checkBoxListener);
            subhbox.add(subsubpanel);

            subsubpanel = new JPanel();
            subsubpanel.setLayout(new GridLayout(2, 1));
            subsubpanel.add(searchInConstantPoolsCheckBoxString = new JCheckBox("字符串常量"));//String Constant
            searchInConstantPoolsCheckBoxString.addItemListener(checkBoxListener);
            subsubpanel.add(searchInConstantPoolsCheckBoxModule = new JCheckBox("Java 模块"));//Java Module
            searchInConstantPoolsCheckBoxModule.addItemListener(checkBoxListener);
            subhbox.add(subsubpanel);

            subpanel = new JPanel();
            subpanel.setBorder(BorderFactory.createTitledBorder("范围"));//Limit To
            subpanel.setLayout(new BorderLayout());
            hbox.add(subpanel);
            ...
            subsubpanel = new JPanel();
            subsubpanel.setLayout(new GridLayout(2, 1));
            subsubpanel.add(searchInConstantPoolsCheckBoxDeclarations = new JCheckBox("声明", true));//Declarations
            searchInConstantPoolsCheckBoxDeclarations.addItemListener(checkBoxListener);
            subsubpanel.add(searchInConstantPoolsCheckBoxReferences = new JCheckBox("引用", true));//References
            ...
            hbox = Box.createHorizontalBox();
            hbox.add(searchInConstantPoolsLabel = new JLabel("匹配类型:"));//Matching types
            hbox.add(Box.createHorizontalGlue());
            vbox.add(hbox);
            ...
            hbox = Box.createHorizontalBox();
            hbox.add(Box.createHorizontalGlue());
            JButton searchInConstantPoolsOpenButton = new JButton("打开");//Open
            ...
            JButton searchInConstantPoolsCancelButton = new JButton("取消");//Cancel
    ...
    public void updateTree(Collection<DelegatingFilterContainer> containers, int matchingTypeCount) {
        SwingUtil.invokeLater(() -> {
            ...
            switch (matchingTypeCount) {
                case 0:
                    searchInConstantPoolsLabel.setText("匹配条目:");//Matching entries
                    break;
                case 1:
                    searchInConstantPoolsLabel.setText("1 个匹配条目:");//matching entry
                    break;
                default:
                    searchInConstantPoolsLabel.setText(matchingTypeCount + " 个匹配条目:");//matching entries
            }
        });
    }
